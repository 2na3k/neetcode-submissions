class Solution:
        ...
        while pq:
            ...
            if step < k and node in adj_list:
                for next_node, weight in adj_list[node]:
                    if weight + cost < node_map[next_node]:
                        nodeMap[nextNode] = weight + cost
                        heapq.heappush(pq,(step+1, nextNode, weight+cost))
        return node_map[dst] if node_map[dst] != float('inf') else -1
    ...
    def findCheapestPrice(self, n, flights, src, dst, k):
        ...
        while pq:
            ...
            # if dst == node:
            #     return cost
            # if step == k or node_map[node]:
            #     continue
            if step < k and node in adj_list:
                for next_node, weight in adj_list[node]:
                    if weight + cost < node_map[next_node]:
                        node_map[next_node] = weight + cost
                        heapq.heappush(pq,(step+1, next_node, weight+cost))
        return node_map[dst] if node_map[dst] != float('inf') else -1
        ...
        while pq:
            ...
            if step <= k and node in adj_list:
                for next_node, weight in adj_list[node]:
                    if weight + cost < node_map[next_node]:
                        node_map[next_node] = weight + cost
                        heapq.heappush(pq, (step + 1, next_node, weight + cost))    # append to the heap
